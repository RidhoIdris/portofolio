...
export default function Portofolio() {
	...
	const Card = ({image,title,link,out = false}) => {
		return(
			<div className="bg-white w-full rounded-lg shadow-lg relative">
				<div>
					<img src={image} alt="byu" className="w-full object-contain object-center rounded-t-lg" />
					<h1 className="py-6 text-center text-gray-500 text-lg font-semibold">{title}</h1>
				</div>
				<div className="transition ease-in duration-200 bg-main-color absolute top-0 w-full h-full opacity-0 hover:opacity-100 rounded-lg flex justify-center items-center">
				{(() => {
					if (out) {
					return (
						<a href={link} rel="noopener noreferrer" target="_BLANK" className="transition-150 border-white border-2 py-1 px-4 hover:bg-white hover:text-main-color text-white font-semibold rounded-full">View Website</a>
					)
					} else {
					return (
	...
	const Porfolio = () => {
		return(
			PortfolioArray.slice(0, visibleBlogs).map((post, index) => (
				<Card key={index} image={post.image} title={post.title} link={post.link} out={post.out} />
			))
		)
	};
    ...
    return (
        <div>
			...
			<div className="w-full px-6 md:px-0 h-auto bg-main-color flex flex-col items-center justify-center pt-20 pb-64">
					<h1 className="md:text-3xl text-2xl text-center text-white font-bold">Hi, I’m Ridho. Nice to meet you.</h1>
			...
			{/* Skill */}
			<div className="flex justify-center px-4 xl:px-40 text-center -mt-40 -md:mb-0 mb-32 flex-col md:flex-row">
			...
			<div className="flex justify-center mb-20 flex-col items-center -md:px-4 px-24">
				...
				{
					PortfolioArray.length !== visibleBlogs ? (
						<button onClick={()=>{setVisibleBlogs(prevVisibleBlogs => prevVisibleBlogs + 3)}} className="transition-150 focus:outline-none mt-16 border-main-color border-2 py-2 px-16 hover:bg-main-color hover:text-white text-main-color font-semibold rounded-full"> Load More</button>
					) : (
						<button onClick={()=>{setVisibleBlogs(3)}} className="transition-150 focus:outline-none mt-16 border-main-color border-2 py-2 px-16 hover:bg-main-color hover:text-white text-main-color font-semibold rounded-full"> Hidden</button>
					)
				}
			</div>
			{/* Portofolio */}
